...
import { ConfirmDeleteDialog } from "../modal/confirm-delete.dialog";
...
const formSchema = z.object({
  ...
  joinDate: z.date({ required_error: "Vui lòng chọn ngày tham gia" }),
  activeDate: z.date({ required_error: "Vui lòng chọn ngày hoạt động" }),
...
export const ParticipantEditForm: React.FC<EditParticipantFormProps> = ({
  defaultValues,
  onClose,
}) => {
  ...
  const handleUpdate = async (data: ParticipantFormValues) => {
    await participationService.updateParticipant(data);
    onModalClose();
    onClose && onClose();
  };

  const handleDelete = async (data: ParticipantFormValues) => {
    await participationService.deleteParticipant(data.participantId);
    onModalClose();
    onClose && onClose();
  };

  // Instead of calling handleDelete directly, we open the confirm dialog.
  const initiateDelete = form.handleSubmit(() => {
...
// de disable cho cac Input tag co the dung readOnly={!editMode}
  return (
    <div className="w-full">
      ...
      <Form {...form}>
        ...
        >
          ...
          {/* Changed to max-w-4xl */}
            <CardHeader>
              <CardTitle className="text-2xl">Chỉnh sửa thông tin Thành viên</CardTitle>
              <CardDescription>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2 md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="NAPAS-001"  {...field}  disabled={true} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2">
                      ...
                      <FormControl>
                        <Input placeholder="DP-001" {...field} disabled={!editMode}  />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2">
                      ...
                      <FormControl>
                        <Input placeholder="+84123456789" {...field} disabled={!editMode} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
      ...
      <ConfirmDeleteDialog
        ...
        onClose={(open : boolean) => setShowConfirmDelete(open)}
        onConfirm={form.handleSubmit(handleDelete)}
      />
    </div>
  );
};
